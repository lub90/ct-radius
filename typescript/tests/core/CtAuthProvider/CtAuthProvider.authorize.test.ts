...
function createProviderWithModules(moduleFactories, route = "wifi") {
    // Install module factories into the registry
    ...
    const cfg = {
        ...
        requestRoutes: {
            wifi: { modules: route === "wifi" ? Object.keys(moduleFactories) : [] },
            vpn: { modules: route === "vpn" ? Object.keys(moduleFactories) : [] }
        },

        // Each module must have a config entry with a type
        ...Object.fromEntries(
            Object.keys(moduleFactories).map(name => [
                name,
                { type: name } // minimal valid module config
            ])
        ),
    ...
    // Mock Config.get() to return this config
    ...
    return new CtAuthProvider("config.json", undefined, route, fakeLogger);
}
...
describe("CtAuthProvider.authorize", () => {
    ...
    for (const route of ["wifi", "vpn"]) {

        // ------------------------------------------------------------
        // 1. One module → must be called
        // ------------------------------------------------------------
        it("calls the module only once", async () => {
            const mod = { authorize: vi.fn().mockResolvedValue(null) };

            const provider = createProviderWithModules({ A: () => mod }, route);
